...
Also, it assumes that there is a function, \pyline{plot_sin_and_cos()} which uses a \pyline{matplotlib}-based package to generate a plot without calling \pyline{plt.savefig()} or \pyline{plt.close()}.
\latexipy\ is known to work well with various libraries, including \pyline{matplotlib}, \pyline{numpy}, \pyline{pandas} and \pyline{seaborn}, among others.

\section{Plotting}
\subsection{Without \latexify}
If you don't \latexify, \pyline{matplotlib}'s defaults are used.
...
To change some parameters, such as the font size, all you need to do is pass a different dictionary to \pyline{lp.latexify()}.
This can be done at any time.
\Cref{lst:new_params} shows an example of increasing the font size.

\begin{listing}[H]
  \pyfile[firstline=84, lastline=89, highlightlines={85, 86, 89}]{examples.py}
...
After the parameters have been updated, running \Cref{lst:sincos_no_latex} again gives \Cref{fig:sincos_big_font}, with a 10\,pt font.
